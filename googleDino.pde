...
void draw() {
  ...
  textSize(100);
  fill(0);
  text("Score: " + dino.score, 5, 100);
  text("High score: " + highScore, width - (500 + (str(highScore).length() * 50)), 100);
}
...
void updateObstacles() {
  ...
  if (!dino.dead) {
    ...
    textSize(100);
    fill(0);
    text("YOU LOSE!!!", width / 2 - 270, height / 2);
    textSize(50);
    text("(Press 'r' or Enter to restart)", width / 2 - 300, height / 2 + 50);
  }
